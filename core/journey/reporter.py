...
class JourneyReporter:
    ...
    def print_measurement_details(self, db: Session, measurement: JourneyMeasurement, indent: str = "") -> None:
        ...
        print(f"{indent}Distance: {float(measurement.distance_meters)/1000:.1f} km ({self.meters_to_miles(float(measurement.distance_meters)):.1f} miles)")
        print(f"{indent}Average Speed: {float(measurement.speed_kph):.1f} kph ({self.kph_to_mph(float(measurement.speed_kph)):.1f} mph)")
        ...
        if measurement.legs:
            ...
            for i, leg in enumerate(measurement.legs, 1):
                ...
                print(f"{indent}Distance: {float(leg.distance_meters)/1000:.1f} km ({self.meters_to_miles(float(leg.distance_meters)):.1f} miles)")
                print(f"{indent}Speed: {float(leg.speed_kph):.1f} kph ({self.kph_to_mph(float(leg.speed_kph)):.1f} mph)")
